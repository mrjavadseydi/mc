...
脚本可以使用 `mcli --json ...` 或 `MC_JSON=true` 获取机器可读输出。启用 JSON
模式后，未知选项、无效的选项值和缺少必需命令参数会在标准输出返回 JSON 错误，
并以状态码 1 退出。请将 `--json` 放在命令之前：解析器遇到无效选项后，
不会继续解析后面的标志。显式指定 `--help` 或 `--version` 时仍显示供人阅读的内容。
`mcli --json admin user` 这类命令组在未指定子命令时，也会显示人类可读的帮助，
并以状态码 0 退出。
